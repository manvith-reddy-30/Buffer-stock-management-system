...
# Constants
APP_NAME = "nalgonda_buffer_report"
USER_ID = "buffer_user"
...
# Define report agent
PROMPT = """
You are a supply chain analyst focused on agricultural planning. Your task is to generate a detailed buffer stock management report for **Nalgonda**, focusing on tomato supply.

You are provided with:
- **Actual tomato prices from the past week** for Nalgonda, Hyderabad, Rangareddy, Medak, and Warangal.
...
Your objectives are:
1. Analyze **Nalgonda’s** tomato price trends using both actual and predicted data.
2. Evaluate **weather conditions** and their potential impact on supply and demand in Nalgonda.
3. Use buffer stock levels and price trends from **neighboring districts** (Hyderabad, Rangareddy, Medak, Warangal) to assess potential:
   - **Risks or opportunities** for Nalgonda’s buffer planning.
...
4. Provide a recommendation on:
   - Whether Nalgonda’s buffer is **sufficient, excessive, or insufficient**.
   - Any **action plan** (e.g., procure more stock, transfer from other regions, reduce storage).
5. Keep the focus of the analysis on Nalgonda, but use the other districts for supporting context.
...
root_agent = LlmAgent(
    name="report_agent",
    description="Analyzes and generates the report for Nalgonda based upon the last week actual tomato prices and next week predicted prices and weather reports",
    instruction=PROMPT,
    model=MODEL,
    tools=[google_search],
...
async def nalgonda_report(
    ...
    query = (
        "**🛑 Nalgonda Tomato Buffer Stock Analysis**\n\n"
