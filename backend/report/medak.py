...
# Constants
APP_NAME = "medak_buffer_report"
USER_ID = "buffer_user"
SESSION_ID = "medak_buffer_session"
session_service = InMemorySessionService()

# Define report agent
PROMPT = """
You are a supply chain analyst focused on agricultural planning. Your task is to generate a detailed buffer stock management report for **Medak**, focusing on tomato supply.

You are provided with:
- **Actual tomato prices from the past week** for Medak, Hyderabad, Nalgonda, Rangareddy, and Warangal.
...
Your objectives are:
1. Analyze **Medak’s** tomato price trends using both actual and predicted data.
2. Evaluate **weather conditions** and their potential impact on supply and demand in Medak.
3. Use buffer stock levels and price trends from **neighboring districts** (Hyderabad, Nalgonda, Rangareddy, Warangal) to assess potential:
   - **Risks or opportunities** for Medak’s buffer planning.
...
4. Provide a recommendation on:
   - Whether Medak’s buffer is **sufficient, excessive, or insufficient**.
   - Any **action plan** (e.g., procure more stock, transfer from other regions, reduce storage).
5. Keep the focus of the analysis on Medak, but use the other districts for supporting context.
...
root_agent = LlmAgent(
    name="report_agent",
    description="Generates a buffer stock report for Medak using price trends, buffer levels, and weather data",
    instruction=PROMPT,
    model=MODEL,
    tools=[google_search],
...
async def medak_report(
    ...
    query = (
        "**🛑 Medak Tomato Buffer Stock Analysis**\n\n"
